...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!scene?.video_url) return publicJson({ error: "Generate this scene's video before approving it" }, { status: 400 });

    await approveAdStudioScene(sceneId);
    return publicJson({ ok: true });
  } catch (err) {
    console.error("ad-studio scene approve failed", err);
    return publicJson({ error: err instanceof Error ? err.message : "Could not approve the scene" }, { status: 500 });
  }
}
