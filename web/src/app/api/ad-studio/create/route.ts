...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user) return publicJson({ error: "Sign in required to use Ad Studio" }, { status: 401 });
    ...
    if (!brief) return publicJson({ error: "Describe what you want before generating a storyboard" }, { status: 400 });
    if (brief.length > MAX_BRIEF_LENGTH) return publicJson({ error: `Brief is too long (max ${MAX_BRIEF_LENGTH} characters)` }, { status: 400 });
    if (!VALID_MODES.includes(mode as AdStudioMode)) return publicJson({ error: "Invalid mode" }, { status: 400 });
    ...
    return publicJson({ projectId, scenes });
  } catch (err) {
